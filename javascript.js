...
function initSubWindow() {
    ...
    $('.sub-window-open-label').on('click', function () {
        ...
        $('#sub-window-content').html($(this).next().prop('outerHTML')).children().addClass('copy toc');
    ...
    // 目次クリックの場合
    $('.toc-content').not('.copy').on('click', function () {
        // コンテンツをコピーし、モーダルを表示する(識別用のクラスも付与)
        if ($('#sub-window').attr('class').indexOf('is-hidden') !== -1) {
             $('#sub-window-content').html('<div>' + $(this).clone(true).html() + '</div>').children().addClass('copy toc article');
